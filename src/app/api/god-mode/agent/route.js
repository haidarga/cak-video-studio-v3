  ...
  gen_image: {
    ...
    handler: async ({ prompt, ar, model: modelOverride, extra_ref_urls, per_image, per_image_labels }, ctx) => {
      ...
      if (ctx.activePersona?.id) {
        ...
        // The persona's AVATAR photo is also a valid identity anchor — include
        // it so a persona with few/no separate refs still locks its face.
        // Without this, ref-to-video had nothing of the persona to go on and
        // rendered a generic person from the prompt's text description.
        if (/^https?:/.test(String(ctx.activePersona.avatar_url || ''))) refUrls.push(ctx.activePersona.avatar_url)
      }
...
- PERSONA OVERRIDES the analysis character: if a persona is ACTIVE (pinned), the video's CHARACTER = that persona (its refs/avatar lock the face — auto-attached by the handler). When replicating an analysis with an active persona, USE the formula for style/camera/pacing/shot-structure/mood, but DROP the analysis's character_notes (hair, glasses, outfit, skin of the REFERENCE person) — do NOT put them in the prompt, they fight the persona's real face. Only describe wardrobe/action, never a conflicting face/hair description.
