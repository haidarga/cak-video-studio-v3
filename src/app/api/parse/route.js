...
export async function POST(req) {
  ...
  const universalRules = [
    ...
    '  • Action / what subjects DO → BOTH image_prompt (single frozen moment) AND video_motion (full action sequence). The image is one still frame; the video has to ANIMATE the action. If you only put action in image_prompt, the video model has nothing to animate and produces a frozen image. Repeat the verb-by-verb action chain in video_motion so the image-to-video model knows what to play out.',
    ...
    'VIDEO MOTION IS NOT JUST CAMERA — IT IS THE FULL ACTION TIMELINE. If the naskah describes a sequence of events (X happens, then Y, then Z), the video_motion field must carry the whole sequence as a beat-by-beat description so the image-to-video model has motion to render. Generic camera-only video_motion ("static CCTV, grainy") produces a frozen image — the user always wants their action played out.',
